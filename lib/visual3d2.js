...
var visualGlob ={
	...
	preInit: function(N_in,step) {
		...
		this.cubedeadmaterial = new THREE.MeshFaceMaterial( [oldmaterial,oldmaterial,oldmaterial,
				oldmaterial,oldmaterial,oldmaterial]);
			...
			buildScene: function(board,step,flag) {
						...
						if (board.grid[i][j] !== 0) {
							if(this.flag){
								if(board.grid[i][j] === 2){
									...
									cube = new THREE.Mesh(this.cubegeometry, this.cubedeadmaterial);
								}
								cube.scale.y = 1;
			...
			render: function(timeFrame,board) {
				...
				if (Date.now() - this.lastStep > timeFrame) {
					if(this.flag){
						...
						this.buildScene(board,50,this.flag);
						this.flag=false;
					}else{
						this.lastStep = Date.now();
						this.buildScene(board,50,this.flag);
